...
class SDOFFARecipe {
    ...
    @Relationship(deleteRule: .cascade, inverse: \SDOFFAIngredients.recipe)
    var extendedIngredients : [SDOFFAIngredients]? = [SDOFFAIngredients]()
    ...
    @Relationship(deleteRule: .cascade, inverse: \SDOFFAAnalyzedInstructions.recipe)
    var analyzedInstructions : [SDOFFAAnalyzedInstructions]? = [SDOFFAAnalyzedInstructions]()
    ...
    init(from recipe : Recipe) {
        ...
        self.extendedIngredients = recipe.extendedIngredients?.map { SDOFFAIngredients(from: $0) } ?? []
        self.analyzedInstructions = recipe.analyzedInstructions?.map { SDOFFAAnalyzedInstructions(from: $0) }
    ...
    init(id: Int, title: String, sourceUrl: String, spoonacularSourceUrl: String, extendedIngredients: [SDOFFAIngredients] = []) {
        self.id = id
        self.title = title

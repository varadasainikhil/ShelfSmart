...
struct HomeView: View {
    ...
    // Filters out groups that have no active (non-used) products
    private var allGroupsSorted: [(date: Date, isOFFA: Bool, index: Int)] {
        var combined: [(date: Date, isOFFA: Bool, index: Int)] = []

        // Add Spoonacular groups (only if they have active products)
        for (index, group) in groups.enumerated() {
            let hasActiveProducts = group.products?.contains { !$0.isUsed } ?? false
            if hasActiveProducts {
                combined.append((date: group.expirationDate, isOFFA: false, index: index))
            }
        }

        // Add OFFA groups (only if they have active products)
        for (index, group) in offaGroups.enumerated() {
            let hasActiveProducts = group.offaProducts?.contains { !$0.isUsed } ?? false
            if hasActiveProducts {

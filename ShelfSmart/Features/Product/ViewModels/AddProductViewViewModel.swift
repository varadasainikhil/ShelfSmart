...
class AddProductViewViewModel {
    ...
    func getAPIKey() -> String? {
        return configManager.spoonacularAPIKey
    }
    
    /// Fetches user's saved allergies from UserService
    ...
    private func fetchUserAllergies(userId: String) async -> [String] {
        do {
            let allergies = try await userService.fetchAllergies(userId: userId)
    ...
    /// First checks Firebase cache, falls back to API if not cached
    func searchProductOFFA(modelContext: ModelContext) async throws {
        ...
        if let cachedData = await ProductFirebaseService.shared.fetchOFFAProductData(barcode: barcode) {
            print("✅ [Firebase] Using cached product data for: \(barcode)")
            
            await MainActor.run {
                self.name = cachedData.name
                self.lastVerifiedBarcode = cachedData.barcode
                self.productDescription = cachedData.description ?? ""
                self.imageLink = cachedData.imageLink ?? ""
                self.searchSuccess = true
                self.isLoading = false
                self.errorMessage = nil
                // Note: offaProduct will be nil since we're using cached data
                // This is fine for the UI, but createOFFAProductFromAPIResponse won't work
                // For cached products, we should still allow saving
            }
            
            print("✅ [Firebase] Product data loaded from cache - ready for user to save")
            analyticsManager.track(.productInfoLoaded, properties: ["api_type": "firebase_cache"])
            return
        }

        // Not in cache, fetch from Open Food Facts API
        print("🔍 [OFFA] Product not in cache, fetching from API...")
        ...
        do {
            ...
            // Upload to Firebase sharedProducts collection (background, non-blocking)
            Task.detached {
                await ProductFirebaseService.shared.saveOFFAProductIfNotExists(product)
            }
